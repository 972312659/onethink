		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><a href="<?php echo U('index?pid='.$list['id']);?>"><?php echo ($list["title"]); ?></a></td>
						<td><?php echo ($list["status"]); ?></td>
						<td><?php echo ($list["price"]); ?></td>
                        <td><?php echo ($list["create_time"]); ?></td>
						<td>
							<a title="编辑" href="<?php echo U('edit?id='.$list['id'].'&pid='.$pid);?>">编辑</a>
							<a href="<?php echo U('setStatus?ids='.$list['id'].'&status='.abs(1-$list['publish']));?>" class="ajax-get"><?php echo (show_status_op($list["publish"])); ?></a>
